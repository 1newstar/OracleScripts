...
rem 23 March 2017.
rem
rem Norman Dunbar  16/01/2018  Removed PDF as Hermes doesn't let me
rem                            use LaTeX. Shame!
rem =================================================================
...
rem See :help_me at the end for colour details.
:get_colour_name
    REM set COLOUR=%2
...
rem
rem 16/01/2018 - Skipped!
rem 
goto :strip_extension
...
:build_docx
    ...
    echo.
    pandoc --from rst --to docx -o %DOCUMENT%.docx --toc --toc-depth=4 --reference-doc=%TEMPLATE_DIR%\pandoc_reference.docx %DOCUMENT%.rst 

    set RESULT=%ERRORLEVEL%
    if "%ERRORLEVEL%" NEQ "0" (
        echo DOCX build error %RESULT%. %DOCUMENT%.docx not built.    
        echo Attempting to build %DOCUMENT%.pdf regardless...
        echo.
    ) else (
        echo Done.
        echo.
...
:build_pdf
    REM echo Building %DOCUMENT%.pdf from %DOCUMENT%.rst - please wait ....
    REM echo Link colours will be %COLOUR% ...
    REM echo.

    REM pandoc -f rst -t latex -o %DOCUMENT%.pdf --toc --toc-depth=4 %DOCUMENT%.rst --variable fontfamily="utopia" --listings -H %TEMPLATE_DIR%\listings_setup.tex --variable toccolor="%TOCCOLOUR%" --variable linkcolor="%LINKCOLOUR%" --variable urlcolor="%URLCOLOUR%" --variable margin-top=2.5cm --variable margin-left=2.5cm --variable margin-right=2.5cm --variable margin-bottom=3.5cm

    set RESULT=%ERRORLEVEL%
    
    REM Bale out early as we are not creating PDFs at Hermes. (Shame!)
    goto :all_done
