...
=========================
    ...
    **This document *may* contain plain text passwords for various accounts and must be treated as strictly confidential.**
...
Oracle patching is, at present, non-existent. All the Azure servers are running the base install of 11.2.0.4. Some testing was done a while back, on ORCDEVORCnn (03 or 04) but as the databases need shutting down to run the updates, nothing has been done elsewhere.

We would need to patch the servers in the following order:

-   devorc01 and devorc02 - the development environments.
-   relorc01 and relorc02 - the release environments.
-   ppduvorc01 and ppduvorc02 - the pre-production environments.
-   uvorc01, uvorc02 and druvorc03 - the production environments.

Obviously, proper testing would be required between phases.


Oracle Enterprise Manager & Monitoring
======================================

The Azure DBAs do not have full access to the OEM system. 

-   We are unable to see certain parts of the system.
-   We cannot install the OEM Agent on a new server as we are not permitted to have access to the account credentials that we need access to.
-   We cannot see what has already been set up to monitor for our Azure databases (although this is becoming clearer!)
-   Etc.

We currently run a number of daily checks against all the Azure databases, plus the production Dimension & Smart Stream databases. We need to automate those and run them viw OEM.

Currently, the Dimension and Smart Stream databases are not monitored by OEM as the OEM Agent isn't installed on the servers, or, they are monitored, but by another team and we have no visibility of same. (Although we apparently support these systems!)


Database Security
=================

We need to investigate who has what access, and why. 

-   There are people in the support team who can deploy scripts on the production databases at will. 

-   There are people who can run code that doesn't COMMIT at the end, but who leave the session uncommitted and leave locks on badly affecting numerous other users.
...
-   Only accounts with APP_USER or DBA profiles have any form of password complexity requirements. Other accounts can have any password they desire, however weak or simple. These profiles do cover the majority of accounts however.
    ...
    Reference: `So How Do You Change A User's Password? <http://qdosmsq.dunbar-it.co.uk/blog/2013/11/so-how-do-you-change-a-users-password/>`_
...
-   The following "ANY" privileges are granted to the listed users. FCS obtains its list from the DBA role mentioned above. 'ANY' privileges have wide ranging effects, when misused. 

The privilege, GRANT ANY ROLE is *very worrying indeed*, as it allows the AURA_ADMIN or FCS users to potentially grant DBA to an otherwise unprivileged user, and revoke it afterwards.
...
-   In the above, AURA_ADMIN has also received the ADMIN option on its grants. This allows AURA_ADMIN to grant its received roles to other users.
...
-   A number of profiles have been created but have not been used, at t he time of writing, by any account:
...
-   OEM monitoring takes place (now) for the Azure databases. However, we still have not received full access to OEM, so are prevented from carrying out certain tasks. We cannot, for example, install the OEM agent on new servers as we are constantly refused access to the require account credentials.

-   [*Previously noted item*] OEM is apparently in use, but recent events have shown that metrics well outside the warning/critical settings - by many magnitudes, were classed as "ok". Example - Recent "logon storm" when a scheduler server (or email server?) went down. Critical was set to around 300 but the actual was into the 20,000 and also 30,0000s - nobody noticed.

    Under normal circumstances, the following should be monitored at the absolute minimum:
  
    -     Backups. Have they succeeded? Have any failed? When was the last successful backup? *Now done in daily checks.*
    -     Tablespace capacity - 80% full means they need looking at. *Now done in daily checks.*
    -     FRA usage. *Now done in daily checks.*
    -     Failed login attempts - could indicate brute force attacks. *Now done in daily checks.*
    -     Failed scheduler jobs.
    -     Accounts with passwords due to expire in the next 14 days, or sooner. *Now done in daily checks.*
    -     Are the standby databases running? Are they suffering from lags in transport or application of logs? *Now done in daily checks.*
    -     Are the OEM agents actually running and communication with OEM?
    -     What errors are reported in the alert log?
    -     Are there any restore points hanging around for too long? These can cause the FRA to fill up. *Now done in daily checks.*
...
-   Have the backup tapes/files ever been used to restore a database to a different server? If not actually restoring, have they ever been used in a "restore validate" to at least prove that they can be read? Has the evidence been documented? Are these tests carried out regularly?
    ...
    **UPDATE**: As part of the staging database refresh procedures, we *always* refresh the AZSTGnn databases from a production database backup. We are therefore continually testing that the backups are usable. We have restored mainly the latest backup (ie, "last night") but we have also restored a backup from a couple of months ago. All work.

-   During the migration to 11g, some user and/or application accounts had their default tablespace set to SYSTEM. This is unacceptable in any Oracle Database, and was rectified as part of the migration itself. 
...
-   A number of tables appear to be over indexed. FCS.INVESTOR and FCS.ORDTRAN specifically, have over 45 different indexes each. Some indexes have numerous columns - many over 10 columns wide.
...
-   At least 21 check constraints in the FCS schema *do not work*. For example:
    ...
    Reference: `Broken Check Constraints Problem <http://qdosmsq.dunbar-it.co.uk/blog/2016/08/dropping-temporary-tables-with-bonus-broken-check-constraints/>`_
